#' 多类型统计模型分析与结果汇总工具
#'
#' 该函数支持多种统计分析（函数参数须知运行经\code{get_Tjstats}），通过配置数据来源、变量筛选、模型参数及输出格式，
#' 生成标准化分析结果并保存到指定环境变量中，尤其适用于批量数据汇总与组间比较场景。
#'
#' @param Stat 字符型，指定分析模型类型。常用值包括"Summary"（默认，描述性统计与组间比较）、
#'   "COX.univariate"（单因素COX回归）、"LOGISTIC"（逻辑回归）、"LINEAR"（线性回归）等，决定分析的核心逻辑。
#' @param Create.Obj 字符型，指定分析结果在全局环境中的存储名称，后续可通过该名称调用结果。默认值为"OA"，
...
#'
#' @return 无显式返回值。分析结果将保存到全局环境中以\code{Create.Obj}命名的列表对象中，包含：
...
#'
#' # 3. 查看结果
#' OA$result.list  # 查看统计结果
#' OA$log          # 查看分析日志
#' }
#'
#' @export
TJ_stats_Summary <- function(
    Stat = "Summary",            #模型类型（如"COX.univariate", "LOGISTIC", "LINEAR"）
    Create.Obj = "Param1",       #输出结果的对象名（用于保存到环境中）
    # 2. 数据配置：输入数据与筛选条件（列表形式）
    data_param = NULL,
    # 3. 变量配置：模型所需的变量（y/x/time等，列表形式）
    var_param = NULL,
    # 4. 模型参数：特定模型的公式与参数（列表形式）
    model_param =  NULL,
    # 5. 输出与评估参数（控制结果呈现）
    output_param =  NULL,
    pack_param= NULL,
  ...
  if (!requireNamespace("httr", quietly = TRUE)) {
    message("httr包未安装，正在为您安装...")
    install.packages("httr", dependencies = TRUE,
                     repos = "https://cloud.r-project.org/")
  }
  library(httr)
  if(is.null(.call)){.call <- "RRR"}else{"RRR"}
  pack_infor <- list(
    .call=.call,
    Stat = Stat ,
  ...
  file_path <- paste0(script_dir, "/send_docu1.rda")
  start_time <- Sys.time()
  analysis.response(file = file_path,obj=Create.Obj)
  end_time <- Sys.time()
  time_elapsed <-  difftime(end_time, start_time, units = "secs")
  # assign(Create.Obj, analysis_result, envir = .GlobalEnv)
  cat("\nTime:",time_elapsed, "秒\n")
  # 打印结果
  unlink(file_path)
  ...
  if(isTRUE(logprint)){
    # 定义更全面的清理函数，处理多种特殊字符
    clean_log <- function(log_output) {
      ansi_regex <- "\\033[[:alnum:];]*"
      log_clean1 <- gsub(ansi_regex, "", log_output)
      cursor_regex <- "[\r\b]"
      log_clean2 <- gsub(cursor_regex, "", log_clean1)
      index_regex <- "\\[\\d+\\] "
      log_clean3 <- gsub(index_regex, "", log_clean2)
      empty_line_regex <- "\n+"
      log_clean4 <- gsub(empty_line_regex, "\n", log_clean3)
      trimws(log_clean4)
    }
    cat(paste(clean_log(get(Create.Obj)[["result.list"]][["log"]]), collapse = "\n"), "\n")
  }else{
