...
Tj_factor_list <- function(factor_df,search_name=T) {
  required_cols <- c("最终变量名", "因子水平", "因子标签")
  ...
  for (var in vars) {
    ...
    if (length(levels) != length(labels)) {
      warning("变量'", var, "'的因子水平与标签数量不匹配，已跳过")
      next
    }
