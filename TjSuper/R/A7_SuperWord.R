...
#' @param introduction 报告正文内容，支持包含统计分析背景、数据来源、分析方法等描述性文本，
...
#'
#' @param text.title.size 报告标题的字号，默认值为14（单位：磅），对应参数含义为“文本.报告标题.字号”。
#' @param text.intro.size 报告正文的字号，默认值为12（单位：磅），对应参数含义为“文本.正文.字号”。
#' @param text.textcom.size 表格前置说明的字号，默认值为12（单位：磅），对应参数含义为“文本.表格前置说明.字号”。
#' @param text.table.caption.size 表格标题的字号，默认值为12（单位：磅），对应参数含义为“文本.表格标题.字号”。
#' @param text.table.content.size 表格内容（含表头）的字号，默认值为11（单位：磅），对应参数含义为“文本.表格内容.字号”。
#' @param text.table.note.size 表注的字号，默认值为10（单位：磅），对应参数含义为“文本.表注.字号”。
#'
#' @param line.title.spacing 报告标题的行距倍数，默认值为1.0（单倍行距），对应参数含义为“行距.报告标题.行距”。
#' @param line.intro.spacing 报告正文的行距倍数，默认值为1.5（1.5倍行距），对应参数含义为“行距.正文.行距”。
#' @param line.textcom.spacing 表格前置说明的行距倍数，默认值为1.5（1.5倍行距），对应参数含义为“行距.表格前置说明.行距”。
#' @param line.caption.spacing 表格标题的行距倍数，默认值为1.0（单倍行距），对应参数含义为“行距.表格标题.行距”。
#' @param line.table.note.spacing 表注的行距倍数，默认值为1.5（1.5倍行距），对应参数含义为“行距.表注.行距”。
#'
#' @param text.title.family 报告标题的字体类型，默认值为`"Times New Roman"`，对应参数含义为“文本.报告标题.字体”。
#' @param text.intro.family 报告正文的字体类型，默认值为`"Times New Roman"`，对应参数含义为“文本.正文.字体”。
#' @param text.textcom.family 表格前置说明的字体类型，默认值为`"Times New Roman"`，对应参数含义为“文本.表格前置说明.字体”。
#' @param text.table.caption.family 表格标题的字体类型，默认值为`"Times New Roman"`，对应参数含义为“文本.表格标题.字体”。
#' @param text.table.content.family 表格内容的字体类型，默认值为`"Times New Roman"`，对应参数含义为“文本.表格内容.字体”。
#' @param text.table.note.family 表注的字体类型，默认值为`"Times New Roman"`，对应参数含义为“文本.表注.字体”。
#'
#'
#' @param pad.table.cell 表格单元格的内边距（即单元格内容与边框的距离），默认值为1（单位：磅），
...
SuperWord <- function(
    ...
    line.title.spacing = 1.0,
    line.intro.spacing = 1.5,
    line.textcom.spacing = 1.5,
    line.caption.spacing = 1.0,
